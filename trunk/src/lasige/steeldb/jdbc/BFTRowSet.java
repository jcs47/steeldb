...
public class BFTRowSet extends CachedRowSetImpl implements RowSet, RowSetInternal, Serializable, Cloneable, CachedRowSet {
...
//	private transient ResultSet resultSet;
	...
	/**
	 * As the resource bundle field is transient, it is not transfered
	 * from the replicas so it needs to be reloaded on the client side
	 */
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + ((rvh == null) ? 0 : rvh.hashCode());
		return result;
	}
	...
	public void populate(ResultSetData data) throws SQLException {
		...
		for(List<Object> row : data.getRows()) {
			...
			for(Object col : row) {
				((Row)currentRow).initColumnObject(i, col);
				i++;
			}
			rowsFetched++;
			rvh.add(currentRow);
	...
	public void restoreOriginal() throws SQLException {
		Row currentRow;
		for (Iterator<BaseRow> i = rvh.iterator(); i.hasNext();) {
	...
	public Collection<?> toCollection() throws SQLException {
		TreeMap<Integer, BaseRow> tMap;
		tMap = new TreeMap<Integer, BaseRow>();
	...
	public Collection<?> toCollection(int column) throws SQLException {
		...
		// create a copy
		BFTRowSet crsTemp;
		crsTemp = (BFTRowSet) this.createCopy();
	...
	public ResultSet getOriginal() throws SQLException {
		...
		for (Iterator<BaseRow> i = rvh.iterator(); i.hasNext();) {
			orig = new Row(colCount, i.next().getOrigRow());
			crs.rvh.add(orig);
		}
...
//		}
//		if( pageSize == 0) {
//			rvh = new Vector<BaseRow>(getMaxRows());
//
//		}
//		else{
//			rvh = new Vector<BaseRow>(getPageSize());
//		}
//
//		if (data == null) {
//			throw new SQLException(resBundle.handleGetObject("cachedrowsetimpl.populate").toString());
...
a
	...
	private java.util.Date parseDate(int columnIndex, Object value)  throws SQLException {
		try {
			...
			java.util.Date dateValue = df.parse(value.toString());
			return dateValue;
