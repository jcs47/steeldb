...
public class BFTPreparedStatement implements PreparedStatement{
	...
	private Object process(int opcode) throws SQLException {
		...
		if(autoGeneratedKeys == Statement.RETURN_GENERATED_KEYS) {
			query = new Message(opcode, current, true, mHandler.getMaster(), autoGeneratedKeys);
                }
                else if ((resultSetType != ResultSet.TYPE_FORWARD_ONLY) || (resultSetConcurrency != ResultSet.CONCUR_READ_ONLY)) {
			query = new Message(opcode, current, true, mHandler.getMaster(), resultSetType, resultSetConcurrency);
                }
                else {
			query = new Message(opcode, current, true, mHandler.getMaster());
                }
		Message reply = null;
