...
//						logger.debug("Message:" + message);
						...
						if(replyMsg == null) {
							...
							if(replyMsg.getContents() instanceof ResultSetData) {
								...
								try {
									...
									bftrs.populate(rsd);
									return new Message(replyMsg.getOpcode(), bftrs, replyMsg.isUnordered(), replyMsg.getMaster(), replyMsg.getAutoGeneratedKeys());
		...
		if(replyContent instanceof ResultSetData) {
			...
			try {
				BFTRowSet bftrs = new BFTRowSet();
				bftrs.populate(rsd);
				reply = new Message(reply.getOpcode(), bftrs, reply.isUnordered(), reply.getMaster(), reply.getAutoGeneratedKeys());                                
			} catch (SQLException e) {
				logger.error("Error populating BFTRowSet", e);
			}
		}
